...
type ParametersOf<T extends ((...args: any) => any) | undefined> = T extends Function
	? Parameters<T>
	: never;
...
export const scopedRegistryPolyfill = () => {
	 ...
	 */
	if (window.CustomElementRegistryPolyfill?.['formAssociated'] === undefined) {
		window['CustomElementRegistryPolyfill'] = {
			['formAssociated']: new Set(),
		};
	}

	// Save original native implementations
	const NativeHTMLElement = window.HTMLElement;
	const nativeDefine = window.customElements.define;
	const nativeGet = window.customElements.get;
	const nativeRegistry = window.customElements;

	const definitionForElement = new WeakMap<HTMLElement, CustomElementDefinition>();
	const pendingRegistryForElement = new WeakMap<HTMLElement, ShimmedCustomElementsRegistry>();
	...
	class ShimmedCustomElementsRegistry implements CustomElementRegistry {
		private readonly _definitionsByTag = new Map<string, CustomElementDefinition>();
		...
		define(tagName: string, elementClass: CustomElementConstructor) {
			...
			const observedAttributes = new Set<string>(elementClass.observedAttributes || []);
			...
			if (awaiting) {
				...
				Array.from(awaiting).forEach((element) => {
		...
		_upgradeWhenDefined(element: HTMLElement, tagName: string, shouldUpgrade: boolean) {
	...
	const isValidScope = (node: Node) => node === document || node instanceof ShadowRoot;
	const registryForNode = (node: Node): ShimmedCustomElementsRegistry | null => {
		...
		if (!isValidScope(scope)) {
			const context = creationContext[creationContext.length - 1]; // document
	...
	const createStandInElement = (tagName: string): CustomElementConstructor => {
		return class ScopedCustomElementBase {
			...
			static get ['formAssociated']() {
				return window['CustomElementRegistryPolyfill']['formAssociated'].has(tagName);
			}
			constructor() {
			...
			connectedCallback(
				...
				// Recursively customize (render) children that are also custom elements
				...
				if (definition) {
					...
					definition.connectedCallback && definition.connectedCallback.apply(this, args);
				} else {
	...
	const patchAttributes = (
		...
		if (toggleAttribute) {
			elementClass.prototype.toggleAttribute = function (n: string, force?: boolean) {
				ensureAttributesCustomized(this);
				const name = n.toLowerCase();
				if (observedAttributes.has(name)) {
					const old = this.getAttribute(name);
	...
	const ensureAttributesCustomized = (instance: CustomHTMLElement & HTMLElement) => {
	...
	const creationContext: Array<Document | CustomElementRegistry | Element | ShadowRoot> = [
		document,
	];
	const installScopedCreationMethod = (ctor: Function, method: string, from?: Document) => {
		const native = (from ? Object.getPrototypeOf(from) : ctor.prototype)[method];
		ctor.prototype[method] = function (this: Element | ShadowRoot, ...args: Array<unknown>) {
			creationContext.push(this);
	...
	if (!!window['ElementInternals'] && !!window['ElementInternals'].prototype['setFormValue']) {
		...
		// const methods: Array<keyof ElementInternals> = ["setFormValue", "setValidity", "checkValidity", "reportValidity"];
		...
		//     // if ((definition as { formAssociated?: boolean })["formAssociated"] === true) {
		...
		//     } catch (e) {
		//       throw new DOMException(
		//         `Failed to execute ${originalMethod} on 'ElementInternals': The target element is not a form-associated custom element. Original error: ${e}`
		//       );
		//     }
		//   };
		// });
		...
		const RadioNodeList = class
			extends Array<HTMLInputElement>
			implements Omit<RadioNodeList, 'forEach'>
		{
			...
			item(index: number): HTMLInputElement {
				return this[index]! as HTMLInputElement;
			}
		...
		const HTMLFormControlsCollection = class implements HTMLFormControlsCollection {
			length: number;
			...
			constructor(elements: Array<HTMLElement>) {
				...
				entries.forEach((value, name) => {
					if (!value) return;
					if (name === 'length' || name === 'item' || name === 'namedItem') return;
					if (value.length === 1) {
						// eslint-disable-next-line @typescript-eslint/no-explicit-any
